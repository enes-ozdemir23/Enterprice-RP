...
namespace Erp
{
    public partial class FrmGenelRapor : Form
    {
        ...
        void stokkontrolraporu()
        {
            conn.Open();
            DataTable dt = new DataTable();
            SqlCommand sorgu1 = new SqlCommand("SELECT STOK_KODU AS 'STOK KODU',STOK_ADI AS 'STOK ADI',(SELECT ISNULL(SUM(MIKTAR),0) FROM TBL_SIPARISKALEMLERI SIP WHERE SIP.STOK_KODU=SK.STOK_KODU AND (URETIMDURUMU='K' OR URETIMDURUMU='B' OR URETIMDURUMU='A')) AS 'SİPARİŞ MİKTARI',(SELECT ISNULL(SUM(MIKTAR),0) FROM TBL_ISEMRI MR WHERE MR.STOK_KODU=SK.STOK_KODU AND DURUM='Y') AS 'İŞ EMRİ MİKTARI',(SELECT ISNULL(SUM(G_MIKTAR)-SUM(C_MIKTAR),0) FROM TBL_STOKHAREKETLERI SH WHERE SH.STOK_KODU=SK.STOK_KODU) AS 'STOK MİKTARI' FROM TBL_STOKKAYITLARI SK", conn);
            SqlDataAdapter da = new SqlDataAdapter(sorgu1);
            da.Fill(dt);
